...
IPTABLES_PROXY_RULE_TEMPLATE="iptables -A PREROUTING -t mangle \\
    -m mac --mac-source MAC_ADDRESS \\
    -m set ! --match-set DIRECT_IPSET dst \\
    -j MARK --set-mark PROXY_MARK"
DNS_UDP_PROXY_RULE_TEMPLATE="iptables -t nat -A PREROUTING \\
    -m mac --mac-source MAC_ADDRESS \\
    -p udp --dport SRC_PORT \\
    -j REDIRECT \\
    --to-ports DST_PORT"
DNS_TCP_PROXY_RULE_TEMPLATE="iptables -t nat -A PREROUTING \\
    -m mac --mac-source MAC_ADDRESS \\
...
do
    echo "$IPTABLES_PROXY_RULE_TEMPLATE" | \
